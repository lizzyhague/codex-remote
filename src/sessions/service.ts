...
export const CODEX_REMOTE_DEVELOPER_INSTRUCTIONS = [
  ...
  "需要交给用户查看的 Markdown 或图片必须保存在当前项目内：正式文件放在它本来应该在的位置；只用于比较、挑选或试验的临时预览放在已被 Git 忽略的 previews/，写入前确认该路径确实被忽略。不要把这类文件放到 ~/.codex、/tmp 或项目外。回复中提供 Markdown 链接，目标为 /view?path= 加 URL 编码后的绝对路径。",
].join("\n\n");
