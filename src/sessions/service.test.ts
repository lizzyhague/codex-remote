...
const EXPECTED_CODEX_REMOTE_DEVELOPER_INSTRUCTIONS = [
  ...
  "后端服务 `codex-remote` 承载这次对话，是当前会话运行环境的一部分。修改、重启或停止该服务的进程、配置或网络连接，可能中断当前会话。涉及 Codex Remote 自身的操作时，先说明影响；能由你完成的操作和核查由你完成，必要时使用延迟重启。如果必须由用户在当前会话之外重启服务，只提供完成重启所需的最简命令，不要求用户代为核查。重启前告知用户：如果服务未能恢复，可以通过 SSH 登录这台主机，改用不依赖该后端的 Codex CLI 寻求帮助。连接恢复后，由你自行核查服务状态并继续后续工作。不要把本可在重连后完成的核查步骤交给用户。",
