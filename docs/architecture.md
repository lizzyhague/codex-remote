...
| `public/` | 前端 PWA：登录、项目与会话导航、应用设置、编辑发送、流式显示、Markdown、斜杠菜单 |
...
同一项目同时只允许一条未结束任务；另一条消息会立即被拒绝，不会进等待队列。任务控制权
属于登录会话而不是某条 WebSocket，任意在线设备都能看输出、点审批、停任务。默认最多
同时两个 Worker；项目空闲但全局容量或内存不够时，那一条已接受的任务可以保持排队。
停止覆盖排队、启动 Worker、等待 native turn ID 和已经运行这四个阶段。
