...
状态持久化；调度器在资源可用后按接收顺序启动。每次启动前通过平台资源适配层读取
保守可用内存预算：Linux 读取 `/proc/meminfo` 的 `MemAvailable`，macOS 解析 `vm_stat`，
用物理内存总量减去确实被占住的页（匿名页、wired 页和压缩器占用页）。两者都把可回收
的文件缓存算作可用，因此语义一致，可以共用同一个门槛值。默认低于 1024 MiB 时继续
排队，而不是冒险把主机推入内存耗尽；只有显式把门槛设为 0 才会关闭这项保护。

平台专用读数失败时（例如 `vm_stat` 输出格式与解析不一致），降级为 `os.freemem()`
并记下原因。降级期间**不按门槛拦截**：读不到内存不等于内存不足，而一次解析失效若
拦住所有会话，连修复所需的会话也开不出来。代价是这段时间没有启动前的内存保护，
因此每次打开新 Worker 都会把降级原因、当前读数和该改哪个文件一并转给浏览器提示，
促成尽早修复。可信读数低于门槛时仍照常拦截，提示写明数值与读数来源，两种情况的
措辞不同，不会互相误导。
