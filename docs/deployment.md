...
没有单独的前端构建命令。服务器在提供页面时自动生成带版本地址的脚本和样式，
保存在 `public/.web-assets/`。运行账户必须能写这个目录；它已被 Git 忽略。
部署新代码时保留该目录，使已经打开的页面仍能获取旧版资源；不要将它提交到仓库。

前端文件仍使用 HTTP 静态资源白名单。新增脚本或样式要登记到
`src/server/http-server.ts` 的白名单，并重启后端；修改已有前端文件无需手工改版本号。
发布文件应作为一套完整改动交付，避免部署过程中对外提供尚未齐全的源码。

联网时，普通刷新获取当前页面及其对应的脚本、样式，不依赖更新提示或 Service Worker
接管。手机可以在聊天列表、会话列表顶部或标题栏空白处下拉，看到「松开刷新」后松手。
页面不会因 Service Worker 更新主动重载。离线时使用最近一次完整缓存的页面，会话仍需联网。
